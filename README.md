...
| 關卡 | 名稱 | 難度 | 碎片形狀 | 縮放模式 | 說明 |
|:---|:---|:---|:---|:---|:---|
| Level 1 | 入門：移動與旋轉 | 簡單 | 正方形 80x80 | 不需要 | 只需調整位置和旋轉 |
| Level 2 | 進階：等比例縮放 | 中等 | 正方形 80x80 | 等比例 | 位置、旋轉、等比縮放 |
| Level 3 | 挑戰：寬高獨立調整 | 困難 | 長方形 120x60, 60x100 | 獨立寬高 | 需要分別調整寬度和高度 |
...
### 工具列
- **Snap 步長選擇**：1px / 5px / 10px（自動調整縮放：1px→3x, 5px→2x, 10px→1x）
- **預覽按鈕**：按住查看目標位置
...
- **自動縮放**：根據精度自動放大畫布（1px→3x, 5px→2x, 10px→1x）
...
- [x] 關卡選擇下拉選單
- [x] 三個漸進難度關卡設計
- [x] Level 1：入門（只調位置和旋轉）
- [x] Level 2：進階（等比例縮放）
- [x] Level 3：挑戰（非等比例，需分別調整寬高）
- [x] 支援自定義碎片形狀（長方形）
- [x] 關卡切換與進度重置

### Phase 9 - 精準對齊工具 ✅
- [x] 自動縮放：根據 snapSize 自動調整畫布放大倍率
- [x] 1px→3x, 5px→2x, 10px→1x
